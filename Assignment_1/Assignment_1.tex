...
\begin{lemma}
\label{lemma 1}
In a quadrilateral ABCD having center O, when the diagonals bisect each other, by "side-angle-side" congruency of the inner triangles, opposite sides of the quadrilateral are equal.
\end{lemma}
...
\begin{lemma}
In a quadrilateral ABCD with center O and having equal opposite sides, by "side-angle-side" congruency, the opposite angle is same as the known angle. \label{lemma 2}
\end{lemma} 
\begin{proof}
...
As the midpoints are equal, they bisect each other. Let the midpoint be $\vec{O}$.
...
From lemma \ref{lemma 1} and lemma \ref{lemma 2}, we have AD = BC and AB = CD
Also, remaining angles are right angles.
...
The center of the rectangle from above is $\vec{O}$ i.e.,
\begin{align}
    \vec{O} = \myvec{8\\ -1}
\end{align}
